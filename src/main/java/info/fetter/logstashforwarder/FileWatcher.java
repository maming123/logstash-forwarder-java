...
public class FileWatcher {
    ...
    public FileWatcher(boolean usingInode) {
            ...
            if (state.getOldFileState() == null) {
                if (usingInode) {
                    for (File otherFile : oldWatchMap.keySet()) {
                        ...
                        if (otherState.getDevice() == state.getDevice() && otherState.getInode() == state.getInode()) {
                            state.setOldFileState(otherState);
                            logger.trace("Same inode size and value : file is the same");
                            break;
                        }
                    }
                } else {
